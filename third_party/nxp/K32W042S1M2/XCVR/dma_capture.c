...
#if RADIO_IS_GEN_3P0
#define RADIO_DMAMUX_CHAN   (8)
#define NUM_DMA_CHANNELS    (8) /* on DMA1 */
#else
#define RADIO_DMAMUX_CHAN   (11)
#define NUM_DMA_CHANNELS    (4) /* on DMA1 */
#endif
...
dma_capture_config_t dma_config;
...
void dma_release(void)
{
    ...
    if (dma_config.osr_temp != 0xF)
    {
        XCVR_RX_DIG->RX_DIG_CTRL &= ~XCVR_RX_DIG_RX_DIG_CTRL_RX_DEC_FILT_OSR_MASK;
        XCVR_RX_DIG->RX_DIG_CTRL |= XCVR_RX_DIG_RX_DIG_CTRL_RX_DEC_FILT_OSR(dma_config.osr_temp);
        dma_config.osr_temp = 0xF;
    }
    ...
    else
    {
        ...
        switch (dma_page)
        {
            ...
            case DMA_PAGE_DEMOD_CFO_PH:
                ...
                {
                    XCVR_RX_DIG->RX_DIG_CTRL &= ~XCVR_RX_DIG_RX_DIG_CTRL_RX_DEC_FILT_OSR_MASK;
                    XCVR_RX_DIG->RX_DIG_CTRL |= XCVR_RX_DIG_RX_DIG_CTRL_RX_DEC_FILT_OSR(new_osr);
                }
...
#endif
                if (new_osr < 0x8) /* need gasket bypass when using 2MHz DMA request rate */
                {
...
void sign_extend_12_16(uint16_t * buffer_ptr, uint32_t count)
{
    ...
    uint32_t i;
    uint16_t * sample_ptr = buffer_ptr;
...
void iq_to_phase_float(uint16_t * iq_buffer, float * phase_buffer, uint16_t num_points)
{
    ...
    assert(phase_buffer != NULL);
    uint16_t i;
    static int16_t i_sample, q_sample;
    uint16_t * in_ptr = iq_buffer;
    float * out_ptr = phase_buffer;
